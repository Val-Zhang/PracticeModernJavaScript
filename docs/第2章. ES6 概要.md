...
ES6对一些已有的功能进行了非破坏性更新，其中的大部分我们可以理解为语法糖，称之为语法糖，意味着，这些新语法能做的事情其实用ES5也可以做，只是会稍微复杂一些。本章我们将着重讨论这些语法糖，看完之后，你一定会对ES6的一些新功能有新理解。考虑到 `let` 和 `const` 从某种层面上可以看做是`var`的拓展，虽然它们不是语法糖，我们也会在在本章中进行讨论。

我在平时使用React的过程中，已经享受到了简写的对象字面量带来的好处，本文也从此开始叙述。
...
ES6 对对象字面量的语法带来了一些改进：包括属性/方法的简洁表示，可计算的属性名等等，我们逐一来看：

### 属性的简洁表示法

你有没有遇到过这种场景，我们声明了一个对象，对象中的若干属性，其值由变量表示，且该变量的名称和属性名是一样的。比如说如下，我们有个名为 `listeners` 的数组，我们想把它赋值给`events`对象中的`listeners`属性，一般说来，用ES5我们会用下面这种写法：
...
怎么样，是不是感觉简洁了许多，ES6让我们在不影响语义的情况下减少了重复代码。

ES6提供了众多让你的代码更简洁，语义更清晰的语法，这只是其中之一，当你习惯了使用这些语法，你的代码的可读性，可维护性，你的开发效率都会大大提升。

### 可计算的属性名

对象字面量的另一个重要更新是允许你使用可计算的属性名，在ES5中我们也可以给对象添加属性名为变量的属性，以下述代码为例，需要这样做，声明一个名为`expertise`的变量，然后通过`person[expertise]`这种形式把变量添加为对象`person`的属性：
...
ES6 中，对象字面量可以使用计算属性名了，把任何表达式放在中括号中，表达式的值就是对应的属性名，针对上面的代码，用ES6可以这样写：
...
不过需要注意的是，**简写属性和计算的属性名不可同时使用**。这是因为，简写属性是一种在编译阶段的就会生效的语法糖，而计算的属性名则需要在运行时生效。如果你把二者混用，代码会报错。而且二者混用往往还会降低代码的可读性，所以在语言层面上限制二者不能混用也是个好事。
...
2. 所构建的对象的属性名来自函数参数。如果使用ES5来处理这种问题，我们需要先声明一个对象字面量，再动态的添加属性，再返回这个对象。下面的例子中，我们创建了一个响应Ajax请求的函数，这个函数的作用在于，请求失败时，返回的对象拥有一个名为`error`属性及对应的描述，请求成功时，该对象拥有一个名为`success`属性及对应的描述。
...
ES6中的对象属性可以简写，方法其实也是可以的。

### 方法定义

以下面代码为例，我们看看传统上如何定义方法，这里我们构建了一个事件发生器，其中的`on`方法用以注册事件，`emit`方法用以执行事件：
...
ES6 运行我们省略对象方法的`function`关键字及之后的冒号，改写后的代码如下：
...
怎么样，有没有感觉使用ES6语法，代码稍微简洁了一些。

ES6中的箭头函数可谓大名鼎鼎了，它有一些特别的优点(关于`this`)，也许你已经很熟悉它了，我也一直在用它，不过有些细节之前却一直不了解，比如箭头函数的几种简写形式及使用注意事项。

## 箭头函数

传统上，JS中声明的函数，需要有函数名，一系列参数和函数体，如下：
...
如果定义的是匿名函数则没有函数名，匿名函数通常会被赋值给一个变量，属性，有时候会被直接调用：
...
ES6 为我们提供了一种写匿名函数的新方法，即使用箭头函数。箭头函数不需要使用`function`关键字，其参数和函数体之间以`=>`相连接：
...
最后一点是箭头函数最大的特点，我们仔细看看这一点。

### 词法作用域

我们在箭头函数的函数体内使用的`this`,`arguments`,`super`等都指向包含箭头函数的作用域，箭头函数本身不产生新的作用域。下述代码中，我们创建了一个名为`timer`的对象，它的属性`seconds`用以计时，方法`start`用以开始计时，若我们在若干秒后调用`start`方法，将打印出当前的`seconds`值。
...
如果这里的`start`方法中，我们没用箭头函数而用的是常规的匿名函数，`this`将指向`window`了，想要让代码正常工作，我们需要在`start`方法开头处插入`var self = this`，然后替换匿名函数函数体中的`this`为`self`。
