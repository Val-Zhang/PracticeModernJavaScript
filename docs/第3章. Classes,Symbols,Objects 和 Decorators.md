...
ES6之前，你可以使用原型链来模拟类的继承，如下代码所示，我们新建了的构造函数`Banana`，用以拓展上文中定义的`Fruit`类，为了`Banana`能够正确初始化,我们需要在`Banana`中调用，`Fruit.call(this, 'banana', 105)`,我们还需要显式的设置`Banana`的`prototype`。
...
鉴于上述代码的复杂性，一般JS开发者都使用库来解决继承问题。比如说Node.js就提供了`util.inherits`。
...
除了banana有确定的`name`和`calories`,以及额外的`slice`方法（用来把banana切为12块）外，`Banana`构造函数和`Fruit`构造函数其实没有区别。下面的代码表现了我们执行`bite`时`Banana`的行为：
...
下面我们用ES6的语法来实现上面的功能，这里我们创建了一个名为`Banana`的类用以继承`Fruit`类。可以看出，这种语法非常清晰，我们无须彻底弄明白原型的机制就可以获得我们想要的结果。我们想要给`Fruit`类传递参数，只需要使用`super`关键字即可。`super`关键字还可以用以调用存在于父类中的方法，比如说`super.chop`,其使用不限于在`constructor`里面。
...
尽管`class`关键字是静态的，结合JavaScript的函数灵活性。所有返回构造函数的表达式都可以用于拓展，我们因此可以创建一个构造函数工厂，并它当做基类。

下面的代码中定义了一个`createJuicyFruit` 的函数，通过使用`super`我们可以给`Fruit`类传入`name`和`calories`,这样可以轻易的拓展`createJuicyFruit`类了。
...
Symbol是ES6提供的一种新的JS基本类型。 它代表唯一值，和字符串，数值等基本类型一个很大的不同点在于Symbol没有字符的表达形式。Symbol的主要目的是用以实现协议，比如说，使用Symbol定义的迭代协议规定了对象将如何被迭代，关于这个，我们将在[Iterator Protocol and Iterable Protocol.]()这一章详细阐述。

Symbol有三种不同类型，其各有不同的使用方法，三种类型如下：
- `local Symbol`；
- `global Symbol`；
- 语言内置`Symbol`；

我们一种种来看它们是什么意思，又该如何使用，首先看`local Symbol`。
...
这里有一点特别值得我们注意，在`Number`或`String`等包装对象前是可以使用`new`操作符的，在`Symbol`前则不能使用，使用了会抛出错误，如下所示：
...
Symbols的类别为`symbol`，使用 `typeof` 可得到其类型返回的值：
...
不过，Symbols绝不是一种用来隐藏属性的安全机制。由特定的方法，它是可见的，如下所示：
...
使用Symbol则完全没有这类问题了，你不用担心定义的属性名会和将来语言中的属性名冲突，它们是唯一的，我们可以按下面这样使用：
...
ES6 提供的一种名为`WeakMap`的新数据类型,它用于唯一地将对象映射到其他对象。和数组查找表比起来，`WeakMap`查找复杂度始终为O(1),我们将在 [Leveraging ECMAScript Collections]() 一章和其它ES6新增数据类型一起讨论。

### 使用符号定义协议

前文中，我们说过 `Symbol` 可以用以定义协议。协议是定义行为的通信契约或约定。简单说来，库可用使用Symbol ,然后依附于该库的对象就可以使用Symbol了。

下述代码中，我们给`character`对象添加了一个`toJSON`方法，用以指定对该对象使用`JSON.stringify`时被序列化的对象。
...
如果`toJSON`不是函数，执行`JSON.stringify`时`character`对象整体将被序列化。
...
如果`toJSON`修饰符是Symbol类型，它就不会影响其它的对象属性了，Symbol本身不调用`Object.getOwnPropertySymbols`永远不会暴露出来的，以下代码中我们用`Symbol`自定义了自定义函数`stringify`：
...
有两种方法可以和全局符号有所交互，`Symbol.for`和`Symbol.keyFor`。我们看看它们分别该如何使用？

#### 通过`Symbol.for(key)`获取symbols

`Symbol.for(key)`方法将在运行时的符号注册表中查找`key`,如果全局注册表中存在`key`则返回其对于的`Symbol`，如果不存在该`key`对于的Symbol，该方法会在全局注册表中创建`key`值为所查找`key`值的Symbol。这意味着，`Symbol.for(key)`是幂等的（多次执行，结果唯一），先进行查找，不存在则新创建，然后返回查找到的或新创建的Symbol。

我们看看使用示例，下面的代码中，第一次调用`Symbol.for`创建了一个key为`example`的Symbol，添加到到注册表，并返回了该Symbol，第二次调用`Symbol.for`由于该`key`已经在注册表中存在，因此返回了之前创建的全局符号。
...
在使用全局符号就像我们使用全局变量一样，合理使用会非常方便，但是不合理使用又会造成灾难。全局符号在符号需要夸代码域使用时非常有用，比如说跨`ServiceWorker`和浏览器页面，但是滥用会导致Symbol难易管理，容易冲突。

下面我们来看，最后一种Symbol，内置的常用Symbol。

### 内置的常用Symbol

内置的常用Symbol为JS语言行为添加了钩子，在一定程度上允许你拓展和自定义ES6语言。
...
我们在[ES6 Essentials]()一章，已经讲述过对象字面量语法的改进，这里我们再补充一下内置对象新增的方法。

除了前面讨论过的`Object.getOwnPropertySymbols`，新增的对象方法还有`Object.assign`,`Object.is`以及`Object.setPrototypeOf`。

### 使用`Object.assign`来拓展对象

现在的前端开发者肯定特别肯定非常数据给中库的使用，一些库在允许我们自定义某些行为的同时也给我们常用的方法属性指定了默认值。

假如说现在有这么一个Markdown库。其接收一个 `input` 参数，依据`input`代表的Markdown内容，转换其为 Html 是默认的用法，用户不需要提供其它参数就可以简单实用这个库。不过，该库还支持多个不同的配置，比如说通过配置可以添加`<script>`或`<iframe>`，也可以启用 css 来高亮渲染代码片段。
...
我们可以使用解构将`defaults`对象设置为`options`的默认值，在本例中，如果用户想要自定义，用户必须提供每个选项的值。
...
`Object.assign` 在这时就可以用来非常方便的合并默认值和用户提供的值，如下代码所示，我们传入`{}`作为`Object.assign`的第一个参数，这个参数在之后将不断依据后面的参数修改，并在最后返回。具体过程如下，我们首先复制defaults到{}中，然后再复制options到前面的结果中，覆盖重复值，最终的结果`config` 对象就会是一个包含默认值和用户自定义值的对象了。
...
> `Object.assign`函数将改变它的第一个参数，它的参数可以归纳为`(target, ...sources)`,所有的 sources 都会被应用到`target`上。

> 如果这里我们的第一个参数不是一个空对象，而是`defaults`，那么`Object.assign()`执行结束之后，`defaults`对象的值也将被改变，虽然这里我们会得到和前面那个例子中一样的结果，但是`default`值被改变，可能也会导致一些意想不到的问题。
...
我们使用如下代码加深你对`Object.assign`的理解：
...
JavaScript中的装饰器语法和Python的非常类似。JavaScript的装饰器可以应用于任何对象或者静态声明的属性。诸如对象字面量声明或`class`声明，甚至它们在`get`,`set`,`static`前。
